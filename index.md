...
There is one fitter per model family, and `sym_type` selects the
symmetric variant within each:

| Function | `sym_type` | Model | Solver |
|----|----|----|----|
| [`psvr_mape()`](https://pbenavidesh.github.io/psvr/reference/psvr_mape.md) | `"none"` | ε-SVR with MAPE | SMO (default) or `osqp` |
| [`psvr_mape()`](https://pbenavidesh.github.io/psvr/reference/psvr_mape.md) | `"even"` / `"odd"` | Symmetric ε-SVR with MAPE | SMO (default) or `osqp` |
| [`psvr_rmspe()`](https://pbenavidesh.github.io/psvr/reference/psvr_rmspe.md) | `"none"` | LS-SVR with RMSPE | linear system |
| [`psvr_rmspe()`](https://pbenavidesh.github.io/psvr/reference/psvr_rmspe.md) | `"even"` / `"odd"` | Symmetric LS-SVR with RMSPE | linear system |

`sym_type = "even"` imposes an even-function prior (`f(-x) = f(x)`),
`"odd"` an odd one. All models require **strictly positive targets**
(`y > 0`).
...
For a full worked example on
[`ggplot2::mpg`](https://ggplot2.tidyverse.org/reference/mpg.html) —
including a 70/30 train–test split, feature standardisation, and
comparison against a linear baseline — see
