...
::: {.callout-note}
## Computational note
The ε-SVR models (Models 1 and 2) solve a quadratic program at each fit,
with cost O(n²) in the number of training observations. Hyperparameter
tuning uses a random subsample of 300 observations to keep compute time
manageable. The final model is fit on the full training set via `last_fit()`.
:::
