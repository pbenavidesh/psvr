...
.extract_pred <- function(preds) {
  if (".pred" %in% names(preds)) {
    ...
    stop(
      "No prediction column found. Names: ",
...
run_seed <- function(X, y, seed, dataset_name) {
  ...
  if (is.null(pred_names)) {
    pred_names <- paste0("V", seq_len(ncol(X)))
  }
  ...
  wf_set <- set_pi(
    ...
    "default_b1",
    cost = cost_param_eps,
    rbf_sigma = rbf_sigma_kernlab
  )
  wf_set <- set_pi(wf_set, "default_b2", mtry = mtry_param_b2)
  wf_set <- set_pi(
    wf_set,
    "default_b3",
    trees = trees_param,
  ...
  score_tuned <- function(mid, model_id, label, abbrev, family, select_metric) {
    wf_id <- paste0("default_", mid)
    ...
    tibble::tibble(
      ...
      dplyr::mutate(
        ...
        epsilon_selected = if ("svm_margin" %in% names(best)) {
          best$svm_margin
        } else {
          NA_real_
        },
        sigma_selected = if ("rbf_sigma" %in% names(best)) {
          best$rbf_sigma
        } else {
          NA_real_
        },
        gamma_selected = if ("rbf_sigma" %in% names(best)) {
          1 / (2 * best$rbf_sigma^2)
        } else {
          NA_real_
        },
        sym_type_selected = if ("sym_type" %in% names(best)) {
          as.character(best$sym_type)
        } else {
          NA_character_
        },
        mtry_selected = if ("mtry" %in% names(best)) best$mtry else NA_integer_,
        xgb_trees_selected = if ("trees" %in% names(best)) {
          best$trees
        } else {
          NA_integer_
        },
        xgb_lr_selected = if ("learn_rate" %in% names(best)) {
          best$learn_rate
        } else {
          NA_real_
        },
        xgb_depth_selected = if ("tree_depth" %in% names(best)) {
          best$tree_depth
        } else {
          NA_integer_
        }
  ...
  results$m1 <- score_tuned("m1", "m1", "SVR-MAPE", "SVR-MAPE", "psvr", "mape")
  results$m2 <- score_tuned(
    "m2",
    "m2",
    "SVR-MAPE + Sym. Kernel",
    "SVR-MAPE+SK",
  ...
  results$b3 <- score_tuned("b3", "b3", "XGBoost", "XGB", "baseline", "rmse")
  ...
  results$b5 <- score_untuned(wf_b5, "b5", "WLS (1/y²)", "WLS", "baseline")
  ...
  b6_out <- tryCatch(
    fit_log_svr_seed(
      ...
      y_tr = y[tr_idx],
      X_te = X[va_idx, , drop = FALSE],
      seed = seed
    ),
    error = function(e) {
      warning(sprintf(
        "[%s] seed %d b6 failed: %s",
        dataset_name,
        seed,
        conditionMessage(e)
      ))
...
run_experiment_parallel <- function(
  X,
  y,
  dataset_name,
  seeds = 1:30,
  ...
  dir.create(partial_dir, showWarnings = FALSE, recursive = TRUE)
    ...
    file.path(partial_dir, sprintf("%s_seed_%02d.rds", dataset_name, seeds))
  )]
  ...
  if (length(tune_present) == length(seeds)) {
    ...
    message(sprintf(
      "[%s] Saved consolidated tune objects: %s",
      dataset_name,
      out_tune_rds
    ))
  } else {
...
summarise_results <- function(
  results,
  metrics = c("MAPE", "RMSPE", "MAAPE", "MASE", "MSE", "R2")
) {
  results |>
