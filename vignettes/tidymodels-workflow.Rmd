...
model evaluation.  We use `psvr_rmspe()` (LS-SVR with RMSPE loss) and tune
the regularisation parameter `cost` ($\Gamma$) against MAPE.
...
`cost` is a `tune()` placeholder here; it maps to the $\Gamma$ regularisation
parameter inside `rmspe_lssvr()`.  The RBF kernel is fixed as an engine
argument.
...
The tidymodels layer wraps a `psvr_rmspe` object. Extract it to use `print()` and `coef()` directly.
