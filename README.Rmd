...
There is one fitter per model family, and `sym_type` selects the symmetric
variant within each:
...
`sym_type = "even"` imposes an even-function prior (`f(-x) = f(x)`), `"odd"` an
odd one. All models require **strictly positive targets** (`y > 0`).
